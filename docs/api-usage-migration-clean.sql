...
create policy "Users can update own usage" on public.api_usage for update using (auth.uid() = user_id);

create policy "Staff can read all usage" on public.api_usage for select using (exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role in ('admin', 'teacher')));
