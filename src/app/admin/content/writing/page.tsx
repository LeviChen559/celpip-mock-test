...
export default function WritingListPage() {
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/admin/content/writing/${id}`, { method: "DELETE" });
    fetchItems();
  };

  const handleTogglePublish = async (id: string) => {
    await fetch(`/api/admin/content/writing/${id}/publish`, { method: "POST" });
    fetchItems();
  };
      ...
      <div className="sticky top-0 z-10 bg-white border-b px-4 py-3">
        <div className="max-w-screen-xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-gray-900">Writing Tasks</h1>
          </div>
          <button
            onClick={() => router.push("/admin/content/writing/new")}
