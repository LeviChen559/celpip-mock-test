import { ReadingPart } from "./celpip-data";

export const madEnglishTask1Parts: ReadingPart[] = [
  // ─── SET 1: Ryan & Charlie (Albania) ───────────────────────────────────────
  {
    id: "Reading-Part1-05",
    title: "05 | 11 Questions | Reading Correspondence",
    paid: true,
    instruction:
      "Read the following emails between two friends. Select the best word or phrase to fill in each blank.",
    passage: `Hi Charlie,

Thanks for your message! I'm happy to hear that you made it safe and sound to Albania. I really enjoyed the pictures you sent. Tirana looks like an awesome city! To be honest, when you first told me you were considering Albania, I thought you were joking. I didn't even know where Albania was. I had to ask my trusty friend Google for help. :) After doing some research, I realized that you made the perfect decision! You've always loved being close to mountains and the beach. I guess now you're a short drive from both! I saw some pictures of the mountains there, and it looks like there are lots of small cement bunkers all over the place. Do you know why that is? I guess I should learn more about Albanian history!

Have you made any local friends to show you around yet? I'm sure staying in the dorm will give you a good opportunity to make lots of friends! I lived in the dorm during my first two years of college, and I have so many great memories of those days. Do you eat at the canteen every day, or are there kitchens in the dorm? I read online that there is a fast food called "suflaqe" which is similar to a doner or a souvlaki. Apparently, they only cost about a dollar! If I were you, I'd be eating those 24/7.

Speaking of food, I've recently taken a job as a pizza delivery guy. I gave my notice at the bank two weeks ago and delivered my first pizzas last night. Talk about a drastic change in lifestyle! I've been wearing a tie to work for the past three years, and now I'll be wearing a Domino Pizza shirt. I don't think it'll take me very long to make the transition though because this lifestyle is what I've been wanting for a long time. I'll have a lot more time to myself from now on. Even though the pay is a lot lower, I'll be much happier because I won't have to deal with stressful situations in the bank, and I'll be able to take up some new hobbies. I'm thinking of getting into snowboarding this winter. Skiing would be good too, but I did a lot of skateboarding when I was a kid, so I think snowboarding might be easier for me.

Anyway, I should sign off here. Hope you have a great time getting set up there!

Take care,
Ryan

---

Hey Ryan,

I'm really happy to hear about the recent changes you made in your life! I always knew a ___ [7] job didn't suit you. You're too creative and independent. Remember in junior high when you started selling M&Ms in math class? I can't believe you made $50 in three days! You've had a/an ___ [8] spirit ever since you were a kid, and I'm sure it'll make you rich someday soon!

You're right, Tirana is a perfect spot for me! The weather is so amazing. It's nice waking up every morning with a view of the mountains, yet smelling the warm air from the ___ [9]. I've heard about the bunkers in the mountains, but I haven't seen them yet. To be honest, I don't really know much about Albania's history either. I'm sure I'll know more soon though. I'll let you know when I ___ [10].

Yes, I've made several friends here, and they've shown me around the city. The school is in a really nice community with lots of nice shops and restaurants nearby. I've eaten at least one suflaqe every day since being here! Even though the cafeteria isn't very expensive, I think it will be even cheaper to ___ [11] every day.

My next class starts in five minutes here, so I better run! Talk to you again soon.

Charlie`,
    questions: [
      {
        id: "Reading-Part1-05-Q1",
        question: "Charlie is in Albania to ___.",
        options: ["work", "study", "travel", "start a business"],
        correctAnswer: 1,
      },
      {
        id: "Reading-Part1-05-Q2",
        question: "Charlie lives ___.",
        options: ["on campus", "off campus", "with a family", "with colleagues"],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-05-Q3",
        question: "When did Ryan stop working at the bank?",
        options: [
          "two weeks ago",
          "yesterday",
          "he did not stop working at the bank",
          "unknown",
        ],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-05-Q4",
        question: "Ryan wanted more ___.",
        options: ["flexibility", "money", "opportunity", "free time"],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-05-Q5",
        question: "Ryan thinks the job transition will be ___.",
        options: ["difficult", "easy", "stressful", "awkward"],
        correctAnswer: 1,
      },
      {
        id: "Reading-Part1-05-Q6",
        question: "Ryan's new hobby will be ___.",
        options: ["snowboarding", "skiing", "skateboarding", "sledding"],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-05-Q7",
        question: "I always knew a ___ job didn't suit you.",
        options: ["white-collar", "blue-collar", "trades", "management"],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-05-Q8",
        question: "You've had a/an ___ spirit ever since you were a kid.",
        options: ["wild", "kind-hearted", "entrepreneurial", "greedy"],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-05-Q9",
        question:
          "It's nice waking up every morning smelling the warm air from the ___.",
        options: ["plateau", "valley", "south", "coast"],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-05-Q10",
        question: "I'll let you know when I ___.",
        options: ["uncover it", "discover it", "find out", "learn about it"],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-05-Q11",
        question:
          "Even though the cafeteria isn't very expensive, I think it will be even cheaper to ___ every day.",
        options: ["eat out", "dine in", "cook my own food", "order in"],
        correctAnswer: 0,
      },
    ],
  },

  // ─── SET 2: Brad & Uncle Denis (Motorbike Trip) ────────────────────────────
  {
    id: "Reading-Part1-06",
    title: "06 | 11 Questions | Reading Correspondence",
    paid: true,
    instruction:
      "Read the following emails between Brad and his Uncle Denis. Select the best word or phrase to fill in each blank.",
    passage: `Dear Uncle Denis,

I just want to say thank you so much for letting me use your motorbike this summer! It was the best summer of my life, and I definitely wouldn't have been able to go on this trip without your help! When my motorbike broke down in May, I was so sad because it meant I couldn't go on the road trip through the States with my friends. We had been planning the trip for most of high school, so it was a crushing blow when the mechanic told me that the repairs would be over $2000. I definitely couldn't afford that. I remember when you called me that evening on May 24th and offered to lend me your motorbike for the whole summer. I was overjoyed! I can't thank you enough for making this trip possible for me!

We were planning to leave Calgary on the morning of the 27th, but it started to pour the night before. We didn't want to start our trip by getting drenched, so we waited till the next morning. Our first stop was Kamloops, BC. It only took us 7 hours to get there, but after we checked into our hotel, I noticed how sore my body was. It took me several days to adjust to being on a new bike! On day two and three, we took it a bit easier and only rode for two hours each day. The weather in BC was so nice! On June 1st, we crossed the boarder into Washington State. It was my first time being in another country. The States is similar in so many ways, yet very different at the same time! One difference is with the measurements. Speed is measured in miles per hour and gas prices are listed in gallons. That confused me for a few days.

We spent a week going down the West Coast, all the way to San Diego. That was probably my favourite part of the trip. It's such a scenic drive! After San Diego, we turned inward to Arizona. The U.S. has such a diverse landscape! It's really hot in that area near the Mexican border. Actually, it's so hot that my buddy's bike overheated, and we had to spend two days in a small town in New Mexico. My buddy Jim was riding his dad's brand new Harley Davidson. I think you would like that bike! I remember you taking me for a ride on your Harley back in the day. Do you miss that bike?

Anyway, the rest of the trip was a blast and we made it back safe and sound. Thanks again so much for your kindness!

Looking forward to seeing you soon,

Brad

---

Dear Brad,

I'm so glad to hear that you had a great trip! It's great that you had some ___ [7]. It would have been really lonely to do that whole trip by yourself. I did a solo trip in my 20s, and, just like you, my favourite place was the ___ [8]. Wow, you made great time to Kamloops. It was probably a good call to ___ [9]. I'm glad you got used to the bike after a few days. You're right, I would have loved ___ [10].

It was my pleasure to ___ [11]. I was so sad when I heard that your bike broke down right before your trip.

Keep being an explorer!

Love,
Uncle Denis`,
    questions: [
      {
        id: "Reading-Part1-06-Q1",
        question: "Brad's motorbike ___.",
        options: [
          "got stolen",
          "had a flat tire",
          "stopped working",
          "ran out of gas",
        ],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-06-Q2",
        question:
          "Brad thinks his uncle would like his friend's motorbike because ___.",
        options: [
          "it had a big engine",
          "it was the same make as one he used to own",
          "it was brand new",
          "it was very fast",
        ],
        correctAnswer: 1,
      },
      {
        id: "Reading-Part1-06-Q3",
        question: "The trip was delayed because ___.",
        options: [
          "it started raining",
          "it started hailing",
          "the roads were too icy",
          "someone's bike needed repairs",
        ],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-06-Q4",
        question: "Brad's body was sore because ___.",
        options: [
          "he fell off the bike",
          "he wasn't used to the bike",
          "the bike was too big",
          "the seat was uncomfortable",
        ],
        correctAnswer: 1,
      },
      {
        id: "Reading-Part1-06-Q5",
        question: "Brad was confused because ___.",
        options: [
          "the gas prices were so high",
          "the speed limit was much lower",
          "they got lost",
          "the USA uses the imperial system of measurement",
        ],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-06-Q6",
        question: "The landscape in the USA is ___.",
        options: [
          "similar to Canada's",
          "uniform",
          "diverse",
          "not good for biking",
        ],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-06-Q7",
        question: "It's great that you had some ___.",
        options: ["company", "setbacks", "precipitation", "detours"],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-06-Q8",
        question: "Just like you, my favourite place was the ___.",
        options: ["desert", "mountains", "prairies", "West Coast"],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-06-Q9",
        question: "It was probably a good call to ___.",
        options: [
          "stay in a hotel",
          "postpone departure by a day",
          "fix up your bike",
          "take a rain check",
        ],
        correctAnswer: 1,
      },
      {
        id: "Reading-Part1-06-Q10",
        question: "You're right, I would have loved ___.",
        options: [
          "to come with you",
          "to see the desert",
          "your buddy's bike",
          "the weather in BC",
        ],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-06-Q11",
        question: "It was my pleasure to ___.",
        options: [
          "borrow your bike",
          "fix your bike",
          "lend you my bike",
          "teach you how to ride",
        ],
        correctAnswer: 2,
      },
    ],
  },

  // ─── SET 3: Jim Baker & Cpl. John Bradley (Police Report) ──────────────────
  {
    id: "Reading-Part1-07",
    title: "07 | 11 Questions | Reading Correspondence",
    paid: true,
    instruction:
      "Read the following police report and the officer's response. Select the best word or phrase to fill in each blank.",
    passage: `To the Calgary Police:

I recently witnessed a violent situation at a restaurant and was asked by Cpl. John Bradley to write a report about what I saw.

At 10:15 a.m., I entered Harvest Hills Cafe and ordered a coffee and muffin. While I was waiting for my order to arrive, I saw two men enter the restaurant and take a seat near the windows. After they placed their orders, they started arguing with each other about something. Their voices grew louder and louder. It was clearly disturbing everyone in the restaurant.

One man was approximately 50 years old and wearing jeans, a navy blue jacket, and a red baseball cap. The other man was approximately 25 years old and wearing brown cargo pants and a green t-shirt.

At approximately 10:25 a.m., I heard the older man shout, "You never pay rent on time you son of a b*tch!" He then punched the younger man in the face. The younger man then started throwing punches as well and knocked the older man out. At approximately 10:30 a.m., two police officers arrived and arrested both men. Before leaving the scene, Cpl. John Bradley asked me to write a report about what I witnessed.

I believe the older man was at fault for the violence because he was the initial aggressor. It seemed as though the younger man was simply responding to the situation; however, there is probably a long backstory to the situation.

After the officers had left the scene, the restaurant manager apologized to all the customers and said everyone's meals would be on the house.

At approximately 10:40 a.m., I left the restaurant.

I hope you find this report useful in your investigation.

Sincerely,
Jim Baker

---

Dear Mr. Baker,

I would like to thank you for your ___ [7]. Your description of the fight confirms why the older man ___ [8]. I agree with you that ___ [9]. I'm happy to hear that ___ [10].

I appreciate your ___ [11].

Sincerely,
Cpl. John Bradley`,
    questions: [
      {
        id: "Reading-Part1-07-Q1",
        question: "The restaurant's customers ___.",
        options: [
          "received a discount on their order",
          "received a coupon for their next visit",
          "were angry about the situation",
          "did not have to pay for their order",
        ],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-07-Q2",
        question: "Jim Baker believes that ___.",
        options: [
          "many things happened leading up to this event",
          "the older man was the victim",
          "both men were at fault",
          "the restaurant should have done more",
        ],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-07-Q3",
        question: "Jim Baker was asked to ___.",
        options: [
          "figure out who was guilty",
          "describe what he saw",
          "give a verbal account of the event",
          "visit the police station",
        ],
        correctAnswer: 1,
      },
      {
        id: "Reading-Part1-07-Q4",
        question: "The older man ___.",
        options: [
          "was wearing a green shirt",
          "ordered soup",
          "became unconscious",
          "knocked out the other man",
        ],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-07-Q5",
        question: "The men were fighting ___.",
        options: [
          "about the cost of utilities",
          "because they were drunk",
          "about a business deal",
          "over the payment of rent",
        ],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-07-Q6",
        question: "Both men ___.",
        options: [
          "were similar in age",
          "were wearing jeans",
          "were sitting near the windows",
          "ordered the same thing",
        ],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-07-Q7",
        question: "I would like to thank you for your ___.",
        options: [
          "summary of the event",
          "verbal account of the event",
          "written account of the event",
          "intervention in the situation",
        ],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-07-Q8",
        question:
          "Your description of the fight confirms why the older man ___.",
        options: [
          "was groggy on our arrival",
          "was chipper on our arrival",
          "apologized for his actions",
          "was unresponsive on our arrival",
        ],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-07-Q9",
        question: "I agree with you that ___.",
        options: [
          "the older man may have started the altercation",
          "there is probably a backstory in this conflict",
          "the younger man was at fault",
          "they should have settled this somewhere else",
        ],
        correctAnswer: 1,
      },
      {
        id: "Reading-Part1-07-Q10",
        question: "I'm happy to hear that ___.",
        options: [
          "you received a free coffee",
          "no customers were injured during the incident",
          "the manager contacted us promptly",
          "none of the windows were broken",
        ],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-07-Q11",
        question: "I appreciate your ___.",
        options: [
          "willingness to make a court appearance",
          "decision to call us right away",
          "courage to defend others at the scene",
          "willingness to provide a written statement",
        ],
        correctAnswer: 3,
      },
    ],
  },

  // ─── SET 4: Tony & Professor Johnson (Late Assignment) ─────────────────────
  {
    id: "Reading-Part1-08",
    title: "08 | 11 Questions | Reading Correspondence",
    paid: true,
    instruction:
      "Read the following student email and professor's response. Select the best word or phrase to fill in each blank.",
    passage: `Dear Professor Johnson,

I would like to apologize for submitting our group assignment several hours late. I know the penalty is a 10% grade reduction for late assignments, but I would like to explain why it was late.

As you know, my group consisted of myself, Cynthia, and Roger. To be honest, I feel very let down by the group. I ended up doing most of the work for the project even though we had discussed the assignment and divided the tasks. My role was to write the introduction and the conclusion and proofread the document before submitting it. I finished writing the introduction and conclusion over a week ago so that the other members would have some structure to build on. I figured they would be working on their parts during the week and then send me their parts on Friday so that I would have all day Saturday to compile everything and make sure it was ready to go.

Unfortunately, my teammates weren't working during the week. On Thursday night, I got a text from Roger saying that he had been busy all week and hadn't started his part yet. He didn't even know where to start, so I ended up doing most of his part of the assignment. Cynthia finally sent me her part on Saturday afternoon, and the writing quality was terrible. It was full of grammar and spelling mistakes. I couldn't believe it. I stayed up all night on Saturday editing her mistakes and writing Roger's part. I finally finished everything just before the deadline but then realized that Cynthia hadn't listed her references. So, I tried to contact her, but it took her two hours to respond to me. Finally, she sent me the references, and I put them into the final assignment.

Needless to say, I'm exhausted. I was trying so hard to get the assignment in on time. Would you be willing to make an exception for our group and overlook the late submission?

Thank you for your time, and I look forward to your response.

Warm regards,
Tony

---

Tony,

I appreciate you letting me know about ___ [7]. It can be very frustrating when your group mates ___ [8]. That's why I provide a "Teammate Evaluation Form" for each group member to fill out anonymously. It takes into account ___ [9]. I would encourage you to fill out the form in a way that reflects your frustrations about the group. This will ___ [10]. I will gladly overlook ___ [11].

Professor Johnson`,
    questions: [
      {
        id: "Reading-Part1-08-Q1",
        question: "Tony is writing the email to ___.",
        options: [
          "complain about his group mates",
          "ask for more time",
          "explain his situation",
          "ask for clarity on the instructions",
        ],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-08-Q2",
        question: "Tony feels ___.",
        options: [
          "disappointed in his group",
          "that there was not enough time for the assignment",
          "Cynthia should lose marks for her bad grammar",
          "Roger should be reprimanded for his lack of effort",
        ],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-08-Q3",
        question: "The last frustration Tony faced was ___.",
        options: [
          "writing Roger's part for him",
          "fixing Cynthia's grammar mistakes",
          "writing the conclusion",
          "asking Cynthia to submit her references",
        ],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-08-Q4",
        question: "Tony finished the introduction and conclusion early ___.",
        options: [
          "so that he could help Roger with his part",
          "so that his group mates would have greater clarity in writing their parts",
          "so that his group mates would finish early too",
          "so that they would not receive the 10% penalty",
        ],
        correctAnswer: 1,
      },
      {
        id: "Reading-Part1-08-Q5",
        question: "Roger ___.",
        options: [
          "finished his part on Thursday evening",
          "asked for more time",
          "edited Cynthia's mistakes",
          "needed Tony's help",
        ],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-08-Q6",
        question: "Tony asked his professor ___.",
        options: [
          "for a higher grade",
          "to overlook the penalty",
          "to punish Roger and Cynthia",
          "to send feedback on his work",
        ],
        correctAnswer: 1,
      },
      {
        id: "Reading-Part1-08-Q7",
        question: "I appreciate you letting me know about ___.",
        options: [
          "your difficulties this week",
          "Cynthia's terrible grammar",
          "the content of your assignment",
          "the upcoming assignment",
        ],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-08-Q8",
        question: "It can be very frustrating when your group mates ___.",
        options: [
          "ask too many questions",
          "do not understand the assignment",
          "fight with each other",
          "do not pull their weight",
        ],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-08-Q9",
        question: "It takes into account ___.",
        options: [
          "the effort of each member",
          "the mistakes of each member",
          "the past assignments of each member",
          "how each member should be punished",
        ],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-08-Q10",
        question: "This will ___.",
        options: [
          "provide you some compensation for your hard work",
          "show your teammates what they did wrong",
          "allow me to adjust individual grades accordingly",
          "carry forward to the next assignment",
        ],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-08-Q11",
        question: "I will gladly overlook ___.",
        options: [
          "the spelling and grammar errors",
          "your group mates' incompetence",
          "the late submission",
          "your request to have the penalty removed",
        ],
        correctAnswer: 2,
      },
    ],
  },

  // ─── SET 5: Greg & John (Wedding / Thailand) ───────────────────────────────
  {
    id: "Reading-Part1-09",
    title: "09 | 11 Questions | Reading Correspondence",
    paid: true,
    instruction:
      "Read the following emails between two old colleagues. Select the best word or phrase to fill in each blank.",
    passage: `Hey John,

Thanks so much for inviting me to your wedding! I got the invitation in the mail today.

To be honest, I was a bit surprised to hear that you decided to get married so soon. I figured you would wait for a while until you got settled into your new job a bit more. How is that going by the way? We all miss you here at the office, but we're happy that you're climbing the corporate ladder! Next stop, CEO, eh?

I'm sorry to say that I won't be able to attend your wedding because I booked my vacation right over those dates. Cindy and I are going to Thailand. We're really excited about the trip. Actually, I'm thinking about proposing on the trip! (Don't let the cat out of the bag!)

We're flying to Bangkok on August 5th. It's gonna be a long trip since we have a 9 hour layover in Tokyo, but it will all be worth it when we get to our beach resort. We're spending two days in Bangkok, then flying to Koh Samui. Cindy has never been to Asia before, so I thought it would be good for her to experience a large Asian city for a few days. We'll be in Koh Samui for a week before flying home. Neither of us have taken a proper vacation in over two years, so it'll be nice to unwind for a while.

I hope you capture some footage of your wedding so that I can watch it after we get back! I'm really excited for you guys. I'll have to keep my eyes open in Bangkok for a belated wedding gift. I picked up an awesome painting from a street artist last time I was in Thailand. Do you have any space on your wall for some artwork?

Anway, I really hope you guys have a great day, and I'm sorry again that I won't be able to make it. Cindy and I will definitely have you guys over for supper when we get back. By the way, have you decided on a honeymoon spot yet? Let me know if you need any advice on good resorts. I have a lot of experience with resorts! Also, my brother is a travel agent and might be able to get you guys a good deal.

Alright, I'll sign off for now.

All the best,
Greg

---

Hi Greg,

Wow, it sounds like you guys have a great trip ahead of you! Don't worry about missing the wedding. I ___ [7]. Actually, April and I are planning to stream our wedding online so that her relatives in Australia can watch it. So, you might be able to ___ [8]. About the artwork, that would be great! I was just telling April yesterday that I want ___ [9].

I definitely miss working with you. How's everything going ___ [10]? My work has been going really well. I have made some great friends in the management team here. The work is a bit more stressful, but ___ [11].

Anway, I look forward to catching up soon!

John`,
    questions: [
      {
        id: "Reading-Part1-09-Q1",
        question: "John and Greg are ___.",
        options: ["classmates", "workmates", "old colleagues", "cousins"],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-09-Q2",
        question: "Cindy is Greg's ___.",
        options: ["wife", "friend", "fiancée", "girlfriend"],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-09-Q3",
        question: "Greg wants John to ___.",
        options: [
          "go to Thailand for his honeymoon",
          "keep his engagement plans a secret",
          "postpone the wedding",
          "apply for a job promotion",
        ],
        correctAnswer: 1,
      },
      {
        id: "Reading-Part1-09-Q4",
        question: "John is ___.",
        options: [
          "a CEO",
          "on his honeymoon",
          "engaged",
          "Greg's boss",
        ],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-09-Q5",
        question: "Greg and Cindy are ___.",
        options: [
          "taking a direct flight",
          "spending a week in Bangkok",
          "getting married in Thailand",
          "connecting in Tokyo",
        ],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-09-Q6",
        question: "Greg wants John to ___.",
        options: [
          "video the wedding",
          "book a resort",
          "send him pictures",
          "save him a seat at the reception",
        ],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-09-Q7",
        question: "Don't worry about missing the wedding. I ___.",
        options: [
          "completely understand",
          "hope you cancel your plans",
          "am pretty upset about it",
          "think you will regret it",
        ],
        correctAnswer: 0,
      },
      {
        id: "Reading-Part1-09-Q8",
        question: "So, you might be able to ___.",
        options: [
          "watch it after you get back",
          "witness the event live from Thailand",
          "fly to Australia to see it",
          "see some pictures after the wedding",
        ],
        correctAnswer: 1,
      },
      {
        id: "Reading-Part1-09-Q9",
        question: "I want ___ in our new apartment.",
        options: [
          "to go to a nice resort for our honeymoon",
          "lots of plants in our new apartment",
          "some nice artwork in our new place",
          "to postpone the wedding",
        ],
        correctAnswer: 2,
      },
      {
        id: "Reading-Part1-09-Q10",
        question: "How's everything going ___?",
        options: [
          "with school",
          "with the wedding plans",
          "at the gym",
          "at the office",
        ],
        correctAnswer: 3,
      },
      {
        id: "Reading-Part1-09-Q11",
        question: "The work is a bit more stressful, but ___.",
        options: [
          "my schedule is not great",
          "I'm happy to be advancing my career",
          "I'm thinking of quitting",
          "that's what you get when you're CEO",
        ],
        correctAnswer: 1,
      },
    ],
  },
];
